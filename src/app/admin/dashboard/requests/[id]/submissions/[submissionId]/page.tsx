...
import { getAdminSingleSubmissionForRequest, getAdminRequest, type Submission, type Request as RequestType, type Question, getAdminClients, type Client } from '@/lib/api';
    ...
    async function fetchSubmissionData() {
      try {
        const [submissionData, requestData] = await Promise.all([
          getAdminSingleSubmissionForRequest(token, requestId, submissionId),
          getAdminRequest(token, requestId),
        ]);
      ...
      <div className="p-6 text-center text-muted-foreground bg-white w-full">
        <h1 className="text-xl font-bold">Submission data could not be loaded.</h1>
         <Button variant="outline" asChild className="mt-4">
            <Link href={`/admin/dashboard/requests/${requestId}`}>
